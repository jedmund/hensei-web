<script lang="ts">
	...
	let { awakening, size = 'medium', showLevel = true }: Props = $props()
	...
	let displayName = $derived(
		awakeningData?.type?.name ? localizedName(awakeningData.type.name) : m.details_awakening()
	)
</script>
...
{#if awakeningData}
	<div class="awakening-display {size}">
		{#if imageUrl}
			<img src={imageUrl} alt={displayName} class="awakening-icon" />
		{:else}
