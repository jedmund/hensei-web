<script lang="ts">
	import { useCreatePlaylist } from '$lib/api/mutations/playlist.mutations'
...
<Dialog bind:open>
	<ModalHeader title={m.playlist_create()} description={m.playlist_create_desc()} />
	...
	<ModalBody>
		<div class="form">
			...
			<fieldset class="fieldset">
				<label class="label" for="playlist-video-url">
					{m.playlist_field_video_url()}
				</label>
				<Input
					id="playlist-video-url"
					bind:value={videoUrl}
	...
	<ModalFooter
		...
		primaryAction={{
			label: createMutation.isPending ? m.playlist_creating() : m.playlist_create(),
