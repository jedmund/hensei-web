...
export class GwAdapter extends BaseAdapter {
	...
	async deleteCrewScore(
		...
		await this.request<void>(`/crew/gw_participations/${participationId}/crew_scores/${scoreId}`, {
			...options,
			method: 'DELETE'
		})
		this.clearCache(`/crew/gw_participations/${participationId}`)
	 ...
	 */
	async getMemberGwScoresByUsername(
		username: string,
		options?: RequestOptions
	): Promise<MemberGwScores> {
		const response = await this.request<MemberGwScores>(
