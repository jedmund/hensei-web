 ...
 */
function transformSettingsResponse(
	apiUser: ApiUserResponse & { hasStoredEditKeys?: boolean }
): UserSettings {
	return {
...
export class UserAdapter extends BaseAdapter {
	...
	async searchUsers(query: string, options?: RequestOptions): Promise<UserInfo[]> {
		const response = await this.request<{ users: ApiUserResponse[] }>('/users/search', {
			...options,
			params: { query }
		})
	 ...
	 */
	async depositEditKeys(
		editKeys: Array<{ shortcode: string; editKey: string }>
