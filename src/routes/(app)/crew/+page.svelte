					...
					<div class="actions">
						...
						{:else}
							<Button
								variant="primary"
								size="small"
								onclick={() => goto(resolvePath(localizeHref('/auth/register')))}
							>
								{m.crew_sign_up()}
							</Button>
							<Button
								variant="secondary"
								size="small"
								onclick={() => goto(resolvePath(localizeHref('/auth/login')))}
							>
								{m.crew_log_in()}
							</Button>
						{/if}
								...
								{#if invitation.crew && invitation.invitedBy}
									<li class="invitation-item">
										...
										<Button
											...
											size="small"
											onclick={() =>
												goto(resolvePath(localizeHref(`/crew/join?invitation=${invitation.id}`)))}
										>
											{m.crew_view()}
										</Button>
									</li>
				...
				{:else if eventsQuery.data && eventsQuery.data.length > 0}
					<ul class="event-list">
						{#each eventsQuery.data as event (event.id)}
							<li
								class="event-item"
								onclick={() => goto(resolvePath(localizeHref(`/crew/events/${event.eventNumber}`)))}
							>
